...
class WordsControllerTest < ActionController::TestCase
  ...
  test "words#index returns array of all words" do
    get :index
    ...
    assert_equal 5, body["words"].length
  end
  ...
  test "words#show a single word" do
    get :show, {id: words(:house).id }
    assert_response :success
    assert_match 'application/json', response.header['Content-Type']
    body = JSON.parse(response.body)
    objects = body["word"]
  ...
  test "words#show returns empty response if the id doesn't exist" do
    word_id = 123456
  ...
  test "words#create adds a new word to the database" do
    word_data = {"form" => "pathan", "translation" => "worry", "language_id" => @kelen_id}
    assert_difference('Word.count', 1) do
      post :create, {"word": word_data}
    end
